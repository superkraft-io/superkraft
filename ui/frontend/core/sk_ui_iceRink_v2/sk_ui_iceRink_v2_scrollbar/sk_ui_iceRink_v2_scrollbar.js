class sk_ui_iceRink_v2_scrollbar extends sk_ui_component {
    constructor(opt){
        super(opt)
        ...
        this.handleContainer = this.add.component(_c => {
            _c.classAdd('sk_ui_iceRink_v2_scrollbar_handleContainer')
            _c.styling = 'left top ttb'
            
            this.handle = _c.add.fromNew(sk_ui_iceRink_v2_scrollbar_handle, _c => {
                _c.onMoving = res => {
        ...
        this.attributes.add({friendlyName: 'Vertical', name: 'vertical', type: 'bool', onSet: val => {
            this.classRemove('sk_ui_iceRink_v2_scrollbar_horizontal')
            this.classRemove('sk_ui_iceRink_v2_scrollbar_vertical')

            if (val){
                this.classAdd('sk_ui_iceRink_v2_scrollbar_vertical')
                this.handle.movable = 'y'
                if (this.visible) this.style.width = ''
                this.style.height = ''
            } else {
                this.classAdd('sk_ui_iceRink_v2_scrollbar_horizontal')
    ...
    set position(percent){
        ...
        if (pos <= 0 - (handleLength - 12)) pos = 0 - (handleLength - 12)
        if (pos >= scrollbarLength - 12) pos = scrollbarLength - 12
...
class sk_ui_iceRink_v2_scrollbar_handle extends sk_ui_component {
    constructor(opt){
        super(opt)
