class sk_ui_1_vst_param_component_draggable extends sk_ui_0_vst_param_component_root {
    constructor(opt) {
        super(opt)
        ...
        var mouseMoveHandler = async _e => {
            if (!this.__sk_ui_vst_param_component_root_mdPos) return
            ...
            if (_e.shiftKey) {
                if (!this.shiftPressed) {
                    ...
                    this.__sk_ui_vst_param_component_root_mdPos = pos
                    this.__sk_ui_vst_param_component_draggable_mdValue = this.value
                }
            } else {
                if (this.shiftPressed) {
                    this.__sk_ui_vst_param_component_root_mdPos = pos
            ...
            var diff = {
                x: pos.x - this.__sk_ui_vst_param_component_root_mdPos.x,
                y: pos.y - this.__sk_ui_vst_param_component_root_mdPos.y
            }
            ...
            this.value = this.__sk_ui_vst_param_component_draggable_mdValue + newVal
            this.applyValue()
        }


        this.__sk_ui_vst_param_component_root_mouseDown = _e => {
            this.__last_mdValue = 0
            this.__sk_ui_vst_param_component_draggable_mdValue = this.value

            this.__sk_ui_vst_param_component_draggable_manuallyChanging = true
        ...
        this.__sk_ui_vst_param_component_root_mouseUp = _e => {
            sk.interactions.unblock()

            delete this.__sk_ui_vst_param_component_draggable_manuallyChanging
    ...
    async readValue() {
        if (this.__sk_ui_vst_param_component_draggable_manuallyChanging || this.__busyReading) return

        this.__sk_ui_vst_param_component_draggable_blockWrite = true
        ...
        this.__sk_ui_vst_param_component_draggable_blockWrite = false
    }
