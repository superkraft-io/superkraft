...
class WebSockets_Callback{
        ...
        */
        ...
        if (this.options.as_iPlug2) {
            ...
            this.sendData = msg => {
                ...
                sk_api.ipc.request('sk_be', msg)
            }

            sk_api.ipc.on('sk.ipc', _msg => {
    ...
    setupAsServer(){
        ...
        if (this.options.asElectron || this.options.asSSC || this.options.asJUCE || this.options.as_iPlug2) return
    ...
    send(message, onResponse = undefined, onProgress = undefined, conn = undefined){
        ...
        if (conn != undefined){
            if (this.options.asElectron || this.options.asSSC || this.options.asJUCE || this.options.as_iPlug2) conn.send(message)
            else conn.send(JSON.stringify(message));
        } else {
            if (this.ws_types.expector == '#s'){
                if (this.options.asElectron || this.options.asSSC || this.options.asJUCE || this.options.as_iPlug2){
                ...
                if (this.options.asElectron || this.options.asSSC || this.options.asJUCE || this.options.as_iPlug2) this.sendData(message)
    ...
    simple(msg, onResponse = undefined, t = undefined){
        var conn = conn
        if (conn != undefined || this.options.asElectron || this.options.asSSC || this.options.asJUCE || this.options.as_iPlug2){
            conn.send({cmd: msg});
        } else {
            if (this.ws_types.expector == '#s'){
                for (var i = 0; i < Object.keys(this.clients).length; i++){
