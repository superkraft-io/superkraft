        ...
        this.parsers = {
            object_v1: (opt)=>{
                ...
                function setupElement(element){
                    element.transition = animation => { return new Promise(resolve => { $(element).transition({animation: animation, duration: 100, onComplete: ()=>{resolve()}}) }) }
                }
                ...
                function parseObject(tree, root, prepend){
                    ...
                    for (var key in tree){
                        ...
                        if (tag.length > 0){
                            ...
                            element.parent = root
                            element.mainBucket = opt.root.bucket

                          
                            if (prepend && firstObject) root.prepend(element)
                ...
                parseObject(opt.tree, opt.root, opt.prepend)
            },
